...
public class PinpadDialog extends BaseDialog {
    ...
    public PinpadDialog(Activity context, String cardno, int type, OnPinpadDialogListener listener) {
        ...
        init(context, cardno, 1, 1, type, 60, true, listener);
    }
    ...
    public PinpadDialog(Activity context, String cardno, int area, int index, int type, int timeout, OnPinpadDialogListener listener) {
        super(context, R.layout.dialog_pay_password, Gravity.BOTTOM, false);
        init(context, cardno, area, index, type, timeout, true, listener);
    }

    public PinpadDialog(Activity context, String cardno, int area, int index, int type, int timeout, boolean isRandom, OnPinpadDialogListener listener) {
        super(context, R.layout.dialog_pay_password, Gravity.BOTTOM, false);
        init(context, cardno, area, index, type, timeout, isRandom, listener);
    }

    private void init(Activity context, String cardno, int area, int index, int type, int timeout, boolean isRandom, OnPinpadDialogListener listener) {
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                initPinPad(cardno, area, index, type, timeout, isRandom);
            }
        }, 400);
...
//		},5000);
    ...
    private void initPinPad(String cardno, int area, int index, int type, int timeout, boolean isRandom) {
        try {
            if (isRandom) {
                ...
                GcSmartPosUtils.getInstance().setPinpadMode(MainApp.getInstance().getApplicationContext(),
                        PosConstants.PinPad.PinpadMode.MODE_FIXED | PosConstants.PinPad.PinpadMode.MODE_NO_BEEP);
            }
